...
- **A real x86-64 migration kernel** — `python build.py check64` builds a genuine ELF64 Mort kernel and three separately linked ELF64 Mort processes. `python test_x86_64.py` proves long mode, GDT/TSS/IDT setup, distinct W^X/NX address spaces, a demand-paged process heap, SYSCALL/SYSRET, validated user pointers, PIDs, cooperative context switching, normal process exit, and scheduler continuation after a deliberate ring-3 page fault. This is the base for the canonical Vex Chromium/Electron port.
