...
- **A real x86-64 migration kernel** — `python build.py check64` builds a genuine ELF64 Mort kernel and a separately linked ELF64 Mort userspace executable. `python test_x86_64.py` proves long mode, GDT/TSS/IDT setup, W^X/NX process mappings, SYSCALL/SYSRET, validated user pointers, normal process exit, and containment of a deliberate ring-3 page fault. This is the base for the canonical Vex Chromium/Electron port.
