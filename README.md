...
- **POSIX socket descriptors** — the native sysroot exposes Linux-compatible `socket` and `getsockopt` entry points for IPv4 streams, including `SO_TYPE`, `SO_ERROR`, nonblocking/cloexec flags, poll/epoll readiness, error translation, and leak-free descriptor cleanup. Connection and stream transfer are the next layer.
