...
- **POSIX TCP and HTTP stream I/O** — the native sysroot exposes Linux-compatible IPv4 stream sockets. A ring-3 C++ program supplies a real `sockaddr_in`, connects to an Internet endpoint, writes an HTTP request, and reads back an `HTTP/` response. Mort validates checksums and sequence numbers, buffers received bytes per socket, advances and acknowledges the stream, handles FIN/RST state, exposes `SO_ERROR` plus poll/epoll readiness, and emits FIN on close.
