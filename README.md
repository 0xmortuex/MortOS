...
- **POSIX TCP connect** — the native sysroot exposes Linux-compatible `socket`, `connect`, and `getsockopt` entry points for IPv4 streams. A ring-3 C++ program supplies a real `sockaddr_in`; Mort validates it, selects a subnet-aware route, resolves the next hop, completes the handshake with per-socket sequence state, and exposes the result through `SO_ERROR` plus poll/epoll readiness. Stream transfer is the next layer.
