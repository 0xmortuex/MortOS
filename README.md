...
- **A real x86-64 migration kernel** — `python build.py check64` builds a genuine ELF64 Mort kernel, four Mort processes, and a separately linked freestanding C++ process. `python test_x86_64.py` proves long mode, GDT/TSS/IDT setup, distinct W^X/NX address spaces, demand-paged heaps, anonymous mappings, an actual RW→RX JIT-code execution transition, C++ constructors/allocation/atomics, shared-address-space user threads with unique TIDs, per-task FS-base TLS restored across scheduling, kernel futex blocking/wakeup, descriptor tables, bounded pipe IPC and event/timed blocking poll, SYSCALL/SYSRET, validated user pointers, PIDs, cooperative and PIT-driven preemptive context switching, interruptible idle, monotonic time, normal process/thread exit, and scheduler continuation after a deliberate ring-3 page fault. This is the base for the canonical Vex Chromium/Electron port.
