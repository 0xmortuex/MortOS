...
- **Event-driven runtime foundation** — installed POSIX `poll`, `epoll`, and `eventfd` headers/libc calls drive pipes and 64-bit event counters; ring-3 tests prove cross-thread wakeups, packed user tokens, interest modification/removal, errno translation, and timeout delivery.
- **POSIX descriptor control** — libc `open`/`openat`, `pipe`/`pipe2`, and `fcntl` expose access modes plus independently mutable `O_NONBLOCK` and `FD_CLOEXEC` state needed by libuv.
- **Native x86-64 Ethernet hardware** — the Mort kernel discovers and enables an RTL8139 through PCI configuration space, installs aligned transmit/receive DMA buffers, reads the hardware MAC, completes a transmit descriptor, then sends an ARP request and validates the gateway's real reply in QEMU.
- **Native IPv4 configuration** — Mort constructs Ethernet/IPv4/UDP/DHCP packets directly, validates the matching DHCP Offer transaction, and records the leased address, default gateway, DNS server, and subnet mask received over the real DMA path.
- **Native DNS resolution** — the kernel ARP-resolves the DHCP-provided DNS server, emits an IPv4/UDP A-record query, validates the matching response, and walks bounded compressed DNS names to extract the answer address.
- **Native TCP transport proof** — Mort ARP-resolves the leased default gateway, sends a checksummed SYN to the resolved Internet host, validates the returned IPv4/TCP lengths, checksum, ports, flags, and sequence acknowledgement, then emits the final ACK. The POSIX socket API and stateful stream engine remain in progress.
- **POSIX TCP and HTTP stream I/O** — the native sysroot exposes Linux-compatible IPv4 stream sockets. A ring-3 C++ program supplies a real `sockaddr_in`, connects to an Internet endpoint, writes an HTTP request, blocks in `epoll_wait`, receives `EPOLLIN` with its preserved user token, and reads back an `HTTP/` response. Mort validates checksums and sequence numbers, buffers received bytes per socket, advances and acknowledges the stream, handles FIN/RST state, exposes `SO_ERROR`, and emits FIN on close.
- **POSIX hostname resolution** — `getaddrinfo`/`freeaddrinfo` accept IPv4 literals or issue a bounded A-record query through the DHCP-provided resolver, construct standard `addrinfo` and `sockaddr_in` results, and parse numeric services. The HTTP regression resolves `example.com` in ring 3 instead of using a fixed address. A checked rotating RTL8139 transmit queue carries all post-boot DNS, ARP, TCP data, ACK, and FIN frames.
- **Libuv-oriented socket controls** — connected streams expose `send`/`recv`, `getsockname`, `getpeername`, `setsockopt` for reuse, buffer, keepalive, and TCP_NODELAY controls, plus read/write half-close through `shutdown`. The regression verifies address identity, MSG_NOSIGNAL transfer, FIN emission, and `EPIPE` after write shutdown.
- **Vectored and message I/O** — libc provides standard `iovec`/`msghdr`, `readv`/`writev`, and connected-stream `sendmsg`/`recvmsg`. The live HTTP regression splits one request across three vectors and receives the response through a message vector, including bounded partial-transfer and errno behavior.
- **POSIX wall and monotonic time** — a Mort CMOS RTC reader anchors `CLOCK_REALTIME` to Unix time while PIT ticks drive `CLOCK_MONOTONIC`; libc exposes `clock_gettime`, `clock_getres`, `nanosleep`, `clock_nanosleep`, `sleep`, and `usleep` over the interruptible scheduler instead of busy-spinning.
- **Kernel wait deadlines** — futex waits can carry a relative deadline and resume with `ETIMEDOUT`, providing the scheduler primitive required for timed pthread and semaphore APIs.
...
- **Reusable bootstrap SDK** — the x86-64 build installs platform headers, `crt1.o`, and `libmortos.a` under `build/x86_64/sysroot`; the boot-tested C++ process is compiled and linked through this sysroot rather than private one-off declarations. The exact libuv 1.52.1 source used by the pinned Node 24.17.0 runtime is vendored as a submodule, and its unmodified public `uv.h` compiles for `x86_64-mortos` against MortOS-owned filesystem, directory, signal, terminal, user, threading, and IPv4/IPv6 ABI headers.
...
- **Protected pthread bootstrap** — `pthread_create` provisions a private stack and FS-based TLS/errno/canary block before invoking ordinary stack-protected C++ workers; futex-backed mutexes, conditions, read/write locks, and POSIX semaphores include realtime absolute deadlines, while blocking `pthread_join` promptly reclaims stack/TLS mappings. One-time initialization, per-thread keys with exit destructors, cross-thread pipes, and epoll also run through this path.
