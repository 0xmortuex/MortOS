...
| `reboot` / `restart` | | Reboots the machine (`reboot`, `kmain.mx:1024`). | `kmain.mx:2541`, `kmain.mx:2545` |
| `shutdown` / `poweroff` | | Powers the machine off — ACPI on emulators, or a halt with an "It is now safe..." message on bare metal (`poweroff`, `kmain.mx:3245`). | `kmain.mx:2549`, `kmain.mx:2553` |
...
| `memtest` | | Runs a RAM test (`memtest`, `kmain.mx:971`). | `kmain.mx:2561` |
