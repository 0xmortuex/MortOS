...
| C/C++ platform | System V `argc`/`argv`/`envp`/auxv startup, freestanding constructors, C allocation, C++ `new`/`delete`, atomics, FS-base TLS, thread-local errno, stack canaries, protected pthread creation, futex-backed mutexes/condition variables, then LLVM target, full libc/libc++, pthread join, build tools | Reusable headers, `crt1.o`, and `libmortos.a` sysroot boot-tested; full platform in progress |
