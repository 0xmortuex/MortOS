...
| Process runtime | Validating ELF64 ET_EXEC loader, W^X PT_LOAD mappings, demand-paged heap, anonymous `mmap`/`mprotect`/`munmap` with executed RW→RX JIT transition, shared-address-space tasks with PID/TID identity, descriptor tables and bounded pipe IPC, cooperative and PIT-preemptive full contexts, terminal address-space reclamation | Boot-tested foundation; futex waits, transferable handles, richer IPC next |
