...
| C/C++ platform | System V `argc`/`argv`/`envp`/auxv startup, freestanding constructors, C allocation, C++ `new`/`delete`, atomics, FS-base TLS, kernel thread primitive, then LLVM target, full libc/libc++, pthread-compatible layer, build tools | Reusable headers, `crt1.o`, and `libmortos.a` sysroot boot-tested; full platform in progress |
