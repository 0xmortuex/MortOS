...
| Node/V8 | V8 JIT permissions, libuv event loop, Node filesystem/network/process APIs | JIT memory, pthread/timers/event-loop foundation, bidirectional Ethernet, ARP, DHCP, DNS A-record resolution, and POSIX IPv4 connect plus epoll-driven buffered HTTP read/write are boot-tested; richer libuv socket calls, retransmission, and the full Node build remain |
