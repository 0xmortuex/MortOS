...
| Node/V8 | V8 JIT permissions, libuv event loop, Node filesystem/network/process APIs | JIT memory, pthread/timers/event-loop foundation, bidirectional Ethernet, ARP, DHCP, DNS A-record resolution, and POSIX IPv4 socket creation/connect/options/poll semantics against a real endpoint are boot-tested; stream transfer and the full Node build remain |
