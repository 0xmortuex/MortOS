...
| C/C++ platform | System V `argc`/`argv`/`envp`/auxv startup, freestanding constructors, C allocation, C++ `new`/`delete`, atomics, FS-base TLS, kernel thread primitive, then LLVM target, full libc/libc++, pthread-compatible layer, build tools | Static bootstrap runtime boot-tested; full platform in progress |
