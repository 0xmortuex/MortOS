...
| Node/V8 | V8 JIT permissions, libuv event loop, Node filesystem/network/process APIs | JIT memory, pthread/timers/event-loop foundation, bidirectional Ethernet, ARP, DHCP, POSIX `getaddrinfo`, and IPv4 connect/send/recv/naming/options/shutdown plus epoll-driven HTTP I/O are boot-tested; vectored/message I/O, retransmission, and the full Node build remain |
