...
| Node/V8 | V8 JIT permissions, libuv event loop, Node filesystem/network/process APIs | JIT memory, pthread/timers/event-loop foundation, bidirectional Ethernet, ARP, DHCP, DNS A-record resolution, a real TCP three-way handshake, and IPv4 socket creation/options/poll semantics are boot-tested; connect/stream transfer and the full Node build remain |
