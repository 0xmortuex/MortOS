# Legacy Vex compatibility prototype

This document describes the earlier Mort-native browser prototype. It is not
the canonical Vex executable and must not be presented as if it were. The
canonical source is
...
This prototype follows some Vex workflows, but its renderer and executable are
different. It remains useful as a small native networking test application
while the actual Vex platform port is built.
