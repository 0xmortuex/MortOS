...
| NIC driver | `net/rtl8139.mx` | An RTL8139 driver: `rtl_init` (`net/rtl8139.mx:64`) finds the card by scanning PCI bus 0 for vendor `0x10EC`/device `0x8139`, enables bus mastering, and programs a receive ring; `rtl_transmit` (`net/rtl8139.mx:175`) fires a TX descriptor; `rtl_poll_rx` (`net/rtl8139.mx:129`) copies one waiting frame (CRC stripped) out of the ring, non-blocking. |
